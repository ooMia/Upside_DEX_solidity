...
interface IDex {
    ...
    function addLiquidity(uint256 amountX, uint256 amountY, uint256 minLPReturn) external returns (uint256 lpAmount);
    ...
    function removeLiquidity(uint256 lpAmount, uint256 minAmountX, uint256 minAmountY)
        external
    ...
    function swap(uint256 amountX, uint256 amountY, uint256 minReturn) external returns (uint256 amount);
}
...
contract Dex is IDex {
    ...
    int256 internal dx;
    int256 internal dy;
    uint256 internal balanceX;
    uint256 internal balanceY;
    ...
    modifier refresh() {
        uint256 bx = tokenX.balanceOf(address(this));
        uint256 by = tokenY.balanceOf(address(this));
        dx = bx != 0 ? bx > balanceX ? int256(bx - balanceX) : -1 * int256(bx - balanceX) : int256(0);
        dy = by != 0 ? by > balanceY ? int256(by - balanceY) : -1 * int256(by - balanceY) : int256(0);
        balanceX = uint256(int256(balanceX) + dx);
        balanceY = uint256(int256(balanceY) + dy);
        _;
    }
        ...
        returns (uint256 lpAmount)
    {
        ...
        require(
            tokenX.balanceOf(msg.sender) >= amountX && tokenY.balanceOf(msg.sender) >= amountY,
