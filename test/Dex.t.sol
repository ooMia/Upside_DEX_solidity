...
contract DexTest is Test {
    ...
    function testAddLiquidity2() external {
        ...
        uint secondLPReturn = dex.addLiquidity(
            1000 ether * 2,
            1000 ether * 2,
            0
        );
    ...
    function testAddLiquidity3() external {
        ...
        (bool success, ) = address(dex).call(
            abi.encodeWithSelector(
                dex.addLiquidity.selector,
                1000 ether,
                1000 ether,
                (firstLPReturn * 10001) / 10000
            )
        );
    ...
    function testAddLiquidity4() external {
        ...
        (bool success, bytes memory alret) = address(dex).call(
            abi.encodeWithSelector(
                dex.addLiquidity.selector,
                1000 ether,
                4000 ether,
        ...
        assertTrue(
            (firstLPReturn == lpret) || !success,
            "AddLiquidity imbalance add liquidity test error"
        );
    }
    ...
    function testAddLiquidity6() external {
        ...
        assertTrue(
            !success,
            "AddLiquidity invalid initialization check error - 1"
        );
        (success, ) = address(dex).call(
            abi.encodeWithSelector(
                dex.addLiquidity.selector,
                1 ether,
                0 ether,
        ...
        assertTrue(
            !success,
            "AddLiquidity invalid initialization check error - 2"
        );
        (success, ) = address(dex).call(
            abi.encodeWithSelector(
                dex.addLiquidity.selector,
        ...
        assertTrue(
            !success,
            "AddLiquidity invalid initialization check error - 3"
        );
    }
    ...
    function testRemoveLiquidity4() external {
        ...
        bool successX = rx <= ((poolAmountX * 10001) / 10000 / 3) &&
            rx >= ((poolAmountX * 9999) / 10000 / 3); // allow 0.01%;
        bool successY = ry <= ((poolAmountY * 10001) / 10000 / 3) &&
            ry >= ((poolAmountY * 9999) / 10000 / 3); // allow 0.01%;
    ...
    function testSwap1() external {
        ...
        int expectedOutput = -(int(poolAmountX * poolAmountY) /
            int(poolAmountX + 300 ether)) + int(poolAmountY);
        ...
        bool success = output <= ((uExpectedOutput * 10001) / 10000) &&
            output >= ((uExpectedOutput * 9999) / 10000); // allow 0.01%;
    ...
    function testSwap2() external {
        ...
        int expectedOutput = -(int(poolAmountY * poolAmountX) /
            int(poolAmountY + 6000 ether)) + int(poolAmountX);
        expectedOutput = (expectedOutput * 999) / 1000; // 0.1% fee
        uint uExpectedOutput = uint(expectedOutput);
    ...
    function testSwap3() external {
        ...
        // check invalid swap
        (bool success, ) = address(dex).call(
            abi.encodeWithSelector(dex.swap.selector, 1, 6000 ether, 0)
        );
    ...
    function testSwap4() external {
        ...
        (bool success, ) = address(dex).call(
            abi.encodeWithSelector(
                ...
                0,
                6000 ether,
                (uExpectedOutput * 1005) / 1000
            )
        );
        assertTrue(
            !success,
            "Swap test fail 4; minimum ouput amount check failed"
        );
    }
